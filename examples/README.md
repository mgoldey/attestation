...
| `mlflow/` | the run ledger reading a real MLflow directory — four sweep arms ranked, one claim checked against them and contradicted on purpose, the same arms retrained in about two seconds | `none — pure local computation` | ~3 s |
...
| `molecular-ai/` | forty-odd canonical molecular-AI papers as a library generated from real APIs by the real readers, synced from one committed `.bib`, searched, walked by citation edge, and counted in the concept graph — offline, with the semantic-vs-substring numbers measured once beside it | `none — pure local computation` | ~5 s |
| `ranking/` | twenty hand-built rows ranked with `rank.rank_rows`, no database — the classifier-only AUC on six labelled clicks beside the rank-order AUC of the blended list, the two numbers `evaluate_user`'s docstring says are different | `none — pure local computation` | ~1 s |
