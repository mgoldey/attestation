...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "kg-report":
        ...
        if kg.is_stale(conn):
            print("\nNOTE: stored graph is stale; `hermes tag` rebuilds it")
        print()
    ...
    if args.command == "ingest":
        ...
        stats = run_ingest(get_db(resolve_db_path(args.db)), Embedder(), args.feeds)
        print(stats)
        return 0
    ...
    if args.command == "tag":
        ...
        stats = attestation.features.run_tagging(get_db(resolve_db_path(args.db)), limit=args.limit)
        print(stats)
    ...
    if args.command == "bootstrap-persona":
        ...
        n = bootstrap_persona(get_db(resolve_db_path(args.db)), Embedder(), args.name, k=args.k)
